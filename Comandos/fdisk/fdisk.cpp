...
void mejor_ajuste(fdisk *particion_comando, Partition particion){
    ...
    int mejor_ajuste = -2;
    for (int i = 0; i < 4; i++){
        if (espacios_libres[i] != NULL && espacios_libres[i] == tamano){
    ...
    if (mejor_ajuste == mbr.mbr_partition_1.part_size && mbr.mbr_partition_1.part_size == -1){
        mbr.mbr_partition_1 = particion;
        fseek(archivo, 0, SEEK_SET);
        fwrite(&mbr, sizeof(MBR), 1, archivo);
        fclose(archivo);
        return;
    } else if (mejor_ajuste == mbr.mbr_partition_2.part_size && mbr.mbr_partition_2.part_size == -1){
        mbr.mbr_partition_2 = particion;
        fseek(archivo, 0, SEEK_SET);
        fwrite(&mbr, sizeof(MBR), 1, archivo);
        fclose(archivo);
        return;
    } else if (mejor_ajuste == mbr.mbr_partition_3.part_size && mbr.mbr_partition_3.part_size == -1){
        mbr.mbr_partition_3 = particion;
        fseek(archivo, 0, SEEK_SET);
        fwrite(&mbr, sizeof(MBR), 1, archivo);
        fclose(archivo);
        return;
    } else if (mejor_ajuste == mbr.mbr_partition_4.part_size && mbr.mbr_partition_4.part_size == -1){
...
void fdisk::make_fdisk(fdisk *partition_comando)
{
    ...
    if ((partition_comando->delete_ == "full" || partition_comando->delete_ == "FULL") && partition_comando->add == NULL){
        delete_partition(partition_comando);
    } else if (partition_comando->add != NULL){
        agregar_espacio(partition_comando);
    } else if (partition_comando->add == NULL && partition_comando->delete_ == ""){
